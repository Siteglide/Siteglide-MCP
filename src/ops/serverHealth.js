...
/** Shown in MCP server instructions and server_health for agents diagnosing client-side timeouts. */
export const MCP_TIMEOUT_GUIDANCE = {
  ...
  firstStep:
    'Call server_health first when unsure. If server_health also times out, report a dead MCP session to the user — do not substitute shell probes for envs_list or other ops tools.',
  userRecovery:
  ...
  commonCause:
    'A prior tool may have blocked the single-threaded stdio server (historically gh auth status without a timeout). After restart, retry server_health then envs_list.',
  filesystemFallback:
...
export const MCP_SERVER_INSTRUCTIONS = [
  ...
  'Call server_health to verify connectivity; if that times out too, tell the user to restart Siteglide MCP or Reload Window.',
...
export function getServerHealth(meta) {
  ...
  return {
    ...
    nextSteps: [
      ...
      'If other tools time out but server_health works, report which tool hung and ask the user to restart MCP.',
      'If server_health times out, follow timeoutGuidance.userRecovery for the user.'
    ]
  };
}
